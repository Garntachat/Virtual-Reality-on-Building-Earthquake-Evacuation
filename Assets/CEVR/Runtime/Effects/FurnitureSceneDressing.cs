...
namespace ChulaEarthquakeVR
{
    public sealed class FurnitureSceneDressing : MonoBehaviour
    {
        ...
        private GameObject TeamModel(string name, Transform parent, Vector3 bottomCenter, Vector3 dimensions, Quaternion rotation)
        {
            ...
            model.transform.position += bottomCenter - new Vector3(placedBounds.center.x, placedBounds.min.y, placedBounds.center.z);
            model.transform.SetParent(parent, true);
            return model;
        }

        private bool ReplaceWithTeamModel(GameObject target, string model, Vector3 dimensions, bool bottomAtPivot = false)
        {
        ...
        private bool ReplaceWithTeamModelAt(GameObject target, string model, Vector3 dimensions, Vector3 bottomCenter)
        {
        ...
        private GameObject TeamDecor(string model, Vector3 bottomCenter, Vector3 dimensions, float yaw = 0f, bool addCollider = false)
        {
        ...
        private void Start()
        {
            ...
            if (top != null)
            {
                ...
                float sceneFloorY = house ? HouseSceneLayout.FloorY : 0f;
                Vector3 bottom = new Vector3(top.transform.position.x, sceneFloorY, top.transform.position.z);
                ...
                Debug.Log($"CEVR {(house ? "House" : "Tutorial")} sturdy table visual grounded at y={sceneFloorY:F2}.");
            }
        ...
        private void DressHouse()
        {
            ...
            TeamDecor("Sofa_Pillows", HouseSceneLayout.Sofa + new Vector3(0.22f, 0.50f, 0f), new Vector3(1.45f, 0.42f, 0.28f), 90f);
            Decor("tableCoffee", HouseSceneLayout.CoffeeTable, new Vector3(1.7f, 0.48f, 0.9f), 90f);
            Decor("rugRectangle", HouseSceneLayout.Rug + Vector3.up * 0.01f, new Vector3(2.5f, 0.02f, 3.0f), 90f);
            Decor("pottedPlant", HouseSceneLayout.Plant, new Vector3(0.7f, 1.35f, 0.7f));
            Decor("cabinetTelevision", HouseSceneLayout.Television, new Vector3(2.0f, 0.65f, 0.5f), -90f);
            Decor("televisionModern", HouseSceneLayout.Television + Vector3.up * 0.65f, new Vector3(1.45f, 0.85f, 0.22f), -90f);
            Decor("kitchenCabinet", HouseSceneLayout.KitchenCabinet, new Vector3(1.05f, 0.9f, 0.65f), 180f);
            Decor("kitchenSink", HouseSceneLayout.KitchenSink, new Vector3(1.05f, 0.9f, 0.65f), 180f);
            Decor("kitchenStove", HouseSceneLayout.KitchenStove, new Vector3(0.9f, 0.9f, 0.65f), 180f);
            Decor("lampRoundFloor", HouseSceneLayout.OnFloor(-4.0f, 1.75f), new Vector3(0.45f, 1.6f, 0.45f));
            TeamDecor("Bed", HouseSceneLayout.Bed, new Vector3(1.80f, 0.72f, 2.40f), 0f, true);
            TeamDecor("Bed_Pillow", HouseSceneLayout.BedPillow, new Vector3(0.72f, 0.18f, 0.42f));
            CreateShakingVase(HouseSceneLayout.CoffeeTable + Vector3.up * 0.49f);
            foreach (Vector3 position in new[] { HouseSceneLayout.OnFloor(-2.8f, 0.6f, 2.5f), HouseSceneLayout.OnFloor(3.4f, 5.2f, 2.5f) })
            {
                ...
                GameObject fixture = Model("lampSquareCeiling", transform, position + Vector3.up * 0.75f, new Vector3(0.8f, 0.15f, 0.5f), Quaternion.identity);
        ...
        private void ValidateHouseFurnitureBounds()
        {
            foreach (Renderer renderer in GetComponentsInChildren<Renderer>(true))
            {
                ...
                if (bounds.center.x < -5.0f || bounds.center.x > 7.0f || bounds.center.z < -7.5f || bounds.center.z > 7.5f)
